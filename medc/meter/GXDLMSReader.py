...
class GXDLMSReader:
    ...
    def __init__(self, client, media, trace, invocationCounter, useOpticalHead):
        ...
        self.client = client
        self.red    = 3
        self.RED    = onionGpio.OnionGpio(self.red)
        status = self.RED.setOutputDirection(0)
        self.meterid_ldn = None
    ...
    def setMeterID_LDN(self, ldn):
        self.meterid_ldn = ldn
    ...
    def readDLMSPacket2(self, data, reply):
        ...
        if self.client.interfaceType == InterfaceType.WRAPPER and isinstance(self.media):
            eop = None
        p = ReceiveParameters()
        p.eop = eop
            ...
            try:
                while not self.client.getData(rd, reply, notify):
                    ...
                    while not self.media.receive(p):
                        ...
                        upload(self.meterid_ldn , "0@0@0@0")
                        for i in range(3):
                            status  = self.RED.setValue(1) 
                            time.sleep(0.2)
                            status  = self.RED.setValue(0) 
                            time.sleep(0.2)
                        status  = self.RED.setValue(1) 
                    rd.set(p.reply)
                    p.reply = None
